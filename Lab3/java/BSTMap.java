...
public class BSTMap<Key extends Comparable<Key>, Value> extends SimpleMap<Key, Value> {
    ...
    public Value get(Key key) {
        ...
        //---------- TASK 3: Implement BST get ------------------------------//
        // Note: If the key does not exist, you should execute the code at the end.

        Value val = getHelper(this.root, key);
    ...
    private Value getHelper(Node subRoot, Key key) {

        if (subRoot == null) return null;

        int keyComp = subRoot.key.compareTo(key);
